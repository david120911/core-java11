...
public class OopFormatTest {


    /**
     * (四) OOP 规约
     * 1. 【强制】避免通过一个类的对象引用访问此类的静态变量或静态方法，
...
//        fileUtils.fileDownload();
     ...
     * 说明:getObject()与 get0bject()的问题。一个是字母的 O，一个是数字的 0，
     * 加@Override 可以准确判断是否覆盖成功。另外，如果在抽象类中对方法签名进行修改，
     * 其实现类会马上编 译报错。
     */

    public void fileDownload() {
        ImageFileUtils imageFileUtils = new ImageFileUtils();
        imageFileUtils.fileDownload();
    }
    ...
    public void setUsers(String type, Integer... ids) {
    ...
    /**
     * new download() is downloadDefined()
     */
     ...
     * 应 该使用双参数 decode(String source, String encode)。接口提供方既然明确是过时接口，
    ...
    //  2) 示例
    public Integer rpcReturn() {
        return new Person().getAge();
    }
    ...
    // 反例 见 Person.java#gmtCreate()

    /**
     *
     * 10. 【强制】序列化类新增属性时，请不要修改 serialVersionUID 字段，
     * 避免反序列失败;如 果完全不兼容升级，避免反序列化混乱，那么请修改 serialVersionUID 值。
     * 说明:注意 serialVersionUID 不一致会抛出序列化运行时异常。
     *
     */

    // 见 Person.java implements Serializable
    ...
    // 见Person.java#Person()  Person.java#init()

    /**
     *
     * 12. 【强制】POJO 类必须写 toString 方法。使用 IDE 的中工具:source> generate toString
     * 时，如果继承了另一个 POJO 类，注意在前面加一下 super.toString。 说明:在方法执行抛出异常时，
     * 可以直接调用 POJO 的 toString()方法打印其属性值，便于排 查问题。
     *
     */
...
// 见Person.toString();
    ...
    // 见Person的三个构造方法
    //见Person.java#loveSports()

    /**
     * 15.【推荐】 类内方法定义顺序依次是:公有方法或保护方法 > 私有方法 > getter/setter 方法。
     * 说明:公有方法是类的调用者和维护者最关心的方法，首屏展示最好;保护方法虽然只是子类 关心，
...
//见Person.java的方法
    ...
    public void stringBuilderForeach() {
        // 正例
    ...
    // 1) 见String2.java
    // 2) 见Person.java#.gender
    // 3) 见Animal.java #setName()
    // 4) 不允许运行过程中重新赋值的局部变量。
    //美女的年龄永远都是18岁
    ...
    // 深拷贝 见 ByteArrayObject 那个案例  二进制读入内存 然后二进制写出一个Object
     ...
     * 或者 一个 public 的成员变量，删除一下，不得手心冒点汗吗?变量像自己的小孩，
     * 尽量在自己的 视线内，变量作用域太大，如果无限制的到处跑，那么你会担心的。
     **/
