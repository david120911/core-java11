...
public class CollectionsTest {
     ...
     * 所以Set存储的 对象必须重写这两个方法。
     * 3) 如果自定义对象做为Map的键，那么必须重写hashCode和equals。
     * 说明:String 重写了 hashCode 和 equals 方法，
     * 所以我们可以非常愉快地使用 String 对象 作为 key 来使用。
     *
     */


    // 1) 见OverrideEquals.java
    // 2) 见RichardMap.java
    // 3) 见RichardSet.java

    // 为何要重新hashcode 因为每个类的hashcode有所不同


    /**
     * 2. 【强制】ArrayList的subList结果不可强转成ArrayList，
     * 否则会抛出ClassCastException 异常:java.util.RandomAccessSubList
     * cannot be cast to java.util.ArrayList ; 说明:subList
     * 返回的是 ArrayList 的内部类 SubList，并不是 ArrayList ，
    ...
    public void subList() {
        ...
        ArrayList arrayList = (ArrayList) list.subList(0, list.size() - 1);
    }


    /**
     * 3. 【强制】 在 subList 场景中，高度注意对原集合元素个数的修改，
     * 会导致子列表的遍历、增 加、删除均产生ConcurrentModificationException 异常。
     */
    ...
    /**
     * 4.【强制】使用集合转数组的方法，必须使用集合的toArray(T[] array)，
     * 传入的是类型完全 一样的数组，大小就是 list.size()。
     * 说明:使用 toArray 带参方法，入参分配的数组空间不够大时，toArray
     * 方法内部将重新分配 内存空间，并返回新数组地址;如果数组元素大于实际所需，
     * 下标为[ list.size() ]的数组 元素将被置为 null，其它数组元素保持原值，
     * 因此最好将方法入参数组大小定义与集合元素 个数一致。
     * 正例:
    ...
    // 正例
    private void collectionCastToArray() {
    ...
    /**
     * 5. 【强制】使用工具类 Arrays.asList()把数组转换成集合时，
     * 不能使用其修改集合相关的方 法，它的 add/remove/clear
    ...
    // 这里案例就不写了
    // 数据结构与算法Java语言描述 里有这个说明 案例以前已经写过了
...
//正例呢？
    ...
    private HashMap hashMap = new HashMap(1024);


    /**
     * 10. 【推荐】使用 entrySet 遍历 Map 类集合 KV，而不是 keySet 方式进行遍历。
     * 说明:keySet 其实是遍历了 2 次，一次是转为 Iterator 对象，另一次是从 hashMap 中取出
     * key 所对应的 value。而 entrySet 只是遍历了一次就把 key 和 value 都放到了
     * entry 中，效 率更高。如果是 JDK8，使用 Map.foreach 方法。
     * 正例:values()返回的是 V 值集合，是一个 list 集合对象;keySet()返回的是 K 值集合，
     * 是 一个 Set 集合对象;entrySet()返回的是 K-V 值组合集合。
     */
...
//        hashMap.forEach(,);
     ...
     *
     *      Value
     * 不允许为 null 不允许为 null 允许为 null 允许为 null
     *
     */
     ...
     * 不稳定性(unorder)带来的负面影响。 说明:有序性是指遍历的结果是按某种比较规则依次排列的。
     * 稳定性指集合每次遍历的元素次 序是一定的。如:ArrayList 是 order/unsort;HashMap 是
     * unorder/unsort;TreeSet 是 order/sort。
     *
     */
//
     ...
     *
     *
     * 13. 【参考】利用 Set 元素唯一的特性，可以快速对一个集合进行去重操作，避免使用
     * List 的 contains 方法进行遍历、对比、去重操作。
     *
     *
     */
